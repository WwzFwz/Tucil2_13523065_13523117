...
void displayUsage(const char* programName) {
    ...
    std::cout << "Options:\n";
    std::cout << "  -i, --input <file>            Input image file (absolute path)\n";
    std::cout << "  -o, --output <file>           Output compressed image file (absolute path)\n";
    ...
    std::cout << "  -g, --gif <file>              Output GIF visualization file (absolute path, optional)\n";
...
int main(int argc, char* argv[]) {
    ...
    if (!interactiveMode) {
        ...
        if (!Utils::isAbsolutePath(inputPath)) {
            inputPath = Utils::toAbsolutePath(inputPath);
        }
        
        if (!Utils::isAbsolutePath(outputPath)) {
            outputPath = Utils::toAbsolutePath(outputPath);
        }
        
        if (!gifPath.empty() && !Utils::isAbsolutePath(gifPath)) {
            gifPath = Utils::toAbsolutePath(gifPath);
        }
    } else {
        ...
        // Input file
        printColoredText("Enter input image path (absolute path): ", GREEN);
        std::getline(std::cin, inputPath);
        if (!Utils::isAbsolutePath(inputPath)) {
            printColoredText("Warning: The input path doesn't appear to be an absolute path.\n", YELLOW);
            printColoredText("Converting to absolute path: " + Utils::toAbsolutePath(inputPath) + "\n", YELLOW);
            inputPath = Utils::toAbsolutePath(inputPath);
        }
        
        // Output file
        printColoredText("Enter output image path (absolute path): ", GREEN);
        std::getline(std::cin, outputPath);
        if (!Utils::isAbsolutePath(outputPath)) {
            printColoredText("Warning: The output path doesn't appear to be an absolute path.\n", YELLOW);
            printColoredText("Converting to absolute path: " + Utils::toAbsolutePath(outputPath) + "\n", YELLOW);
            outputPath = Utils::toAbsolutePath(outputPath);
        }
        ...
        // Get default threshold based on selected method
        double defaultThreshold = Utils::getDefaultThreshold(errorMethod);
        
        // Threshold
        printColoredText("\nEnter error threshold [default: " + std::to_string(defaultThreshold) + "]: ", GREEN);
        ...
        // GIF path
        printColoredText("Enter GIF visualization path (absolute path, leave empty to skip): ", GREEN);
        std::getline(std::cin, gifPath);
        if (!gifPath.empty() && !Utils::isAbsolutePath(gifPath)) {
            printColoredText("Warning: The GIF path doesn't appear to be an absolute path.\n", YELLOW);
            printColoredText("Converting to absolute path: " + Utils::toAbsolutePath(gifPath) + "\n", YELLOW);
            gifPath = Utils::toAbsolutePath(gifPath);
        }
    ...
    if (!success) {
        std::cerr << "Failed to load the input image.\n";
        return 1;
    }
    
    success = processor.compressImage();
    if (!success) {
        std::cerr << "Failed to compress the image.\n";
        return 1;
    }
    
    success = processor.saveCompressedImage();
    if (!success) {
        std::cerr << "Failed to save the compressed image.\n";
        return 1;
    }
